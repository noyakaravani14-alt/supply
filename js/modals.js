...
function holdersDatalistHtml(){
  return `<datalist id="holdersDatalist">${state.holders.map(h=>`<option value="${escAttr(h.name)}">`).join('')}</datalist>`;
}

function autofillHolderEmail(prefix, i){
  const nameId = i===''? `${prefix}_holder` : `${prefix}_holder_${i}`;
  const emailId = i===''? `${prefix}_email` : `${prefix}_email_${i}`;
  const nameEl = document.getElementById(nameId);
  const emailEl = document.getElementById(emailId);
  if(!nameEl || !emailEl) return;
  if(emailEl.value.trim()) return;
  const h = state.holders.find(x=>x.name===nameEl.value.trim());
  if(h && h.email) emailEl.value = h.email;
}
...
async function submitReceive(){
  ...
  if(dest === '__warehouse__'){
    ...
    for(let i=0;i<qty;i++){
      ...
      const resolved = await resolveHolder(holderName, holderEmailInput);
      const unit = {
...
function editUnit(loc, unitId){
  ...
  openModal(`
    ...
    <div class="row2">
      <div class="field"><label>אימייל בעל תפקיד</label><input id="eu_email" type="text" value="${escAttr(u.holderEmail)}"></div>
...
async function saveUnitEdit(loc, unitId){
  ...
  u.holderName = holderName;
  u.holderEmail = resolved.email || holderEmailInput;
